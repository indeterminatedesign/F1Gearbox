...
SX1509 sx1509; // Create an SX1509 object
...
void SevSeg::begin(byte hardwareConfig, byte numDigitsIn, byte digitPinsIn[],
  ...
  for (byte digit = 0 ; digit < numDigits ; digit++) {
    sx1509.pinMode(digitPins[digit], OUTPUT);
    sx1509.digitalWrite(digitPins[digit], digitOffVal);
  }

  for (byte segmentNum = 0 ; segmentNum < numSegments ; segmentNum++) {
    sx1509.pinMode(segmentPins[segmentNum], OUTPUT);
    sx1509.digitalWrite(segmentPins[segmentNum], segmentOffVal);
  }

  blank(); // Initialise the display
}

bool SevSeg::IOBegin(byte address)
{
  sx1509.pinMode(14,ANALOG_OUTPUT);
  sx1509.pinMode(15,ANALOG_OUTPUT);
  sx1509.pinMode(6,ANALOG_OUTPUT);
  sx1509.pinMode(7,ANALOG_OUTPUT);
    return sx1509.begin(address);
...
void SevSeg::segmentOn(byte segmentNum) {
  sx1509.digitalWrite(segmentPins[segmentNum], segmentOnVal);
  for (byte digitNum = 0 ; digitNum < numDigits ; digitNum++) {
    if (digitCodes[digitNum] & (1 << segmentNum)) { // Check a single bit
      sx1509.digitalWrite(digitPins[digitNum], digitOnVal);
    }
  }
}
...
void SevSeg::segmentOff(byte segmentNum) {
  for (byte digitNum = 0 ; digitNum < numDigits ; digitNum++) {
    sx1509.digitalWrite(digitPins[digitNum], digitOffVal);
  }
  sx1509.digitalWrite(segmentPins[segmentNum], segmentOffVal);
}
